...
public class RssParser extends GeneralParser {
	...
	public feed.Feed parser(String archXML) {
		...
		try {
			...
			System.out.println("Root element name is "+elementO.getTagName());
	...
	public feed.Feed parser(String archXML) {
		...
		try {
			...
			*/
			...
			Element elementO = doc.getDocumentElement();
			System.out.println("Root element name is "+elementO.getTagName());
			feed.Feed f = new feed.Feed("ALGUN NOMBRE ");
			...
			for (int i = 0; i < listItems.getLength(); i++) {
				...
				if (node.getNodeType() == Node.ELEMENT_NODE) {
					...
					String desc = null;
					try{
						desc= element.getElementsByTagName("subtitle").item(0).getTextContent();
					}catch(NullPointerException e){desc= "Sin descripcion"; }
					String date = element.getElementsByTagName("updated").item(0).getTextContent();
					String link = element.getElementsByTagName(LINK).item(0).getTextContent();
					Date fecha = parsingDate(date);
